describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const root = await prod.getProperty('shadowRoot');
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (p of items) {
      const root = await p.getProperty('shadowRoot');
      const btn = await root.$('button');
      await btn.click();
    }
    const count = await page.$eval('#cart-count', e => e.innerText);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
     ...
     */
    await page.reload();
    expect(await page.$eval('#cart-count', e => e.innerText)).toBe('20');
    const items = await page.$$('product-item');
    for (p of items) {
      const root = await p.getProperty('shadowRoot');
      const btn = await root.$('button');
  ...
  it('Checking number of items in cart on screen after reload', async () => {
     ...
     */
    await page.reload();
    expect(await page.$eval('#cart-count', e => e.innerText)).toBe('0');
    const items = await page.$$('product-item');
    for (p of items) {
      const root = await p.getProperty('shadowRoot');
      const btn = await root.$('button');
